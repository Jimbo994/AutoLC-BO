...
class MaternSortKernel(Kernel):
    ...
    * :math:`K_\nu` is a modified Bessel function.
    ...
    def __init__(self, nu=2.5, **kwargs):
        ...
        super(MaternSortKernel, self).__init__(**kwargs)
        self.nu = nu
    ...
    def forward(self, x1, x2, diag=False, **params):
        ...
        if len(x1.shape) == 3:
            ...
            if dim == 2 :
                ...
                x1_sorted = x1.clone()
                x2_sorted = x2.clone()
                x1_sorted[:, :, :int(dim/2)], _ = torch.sort(x1[:, :, :int(dim/2)])
                x2_sorted[:, :, :int(dim/2)], _ = torch.sort(x2[:, :, :int(dim/2)])
                # Only for new time setup, also sorts time parameters from low to high
                x1_sorted[:, :, int(dim / 2):], _ = torch.sort(x1[:, :, int(dim / 2):])
                x2_sorted[:, :, int(dim / 2):], _ = torch.sort(x2[:, :, int(dim / 2):])
                x1 = x1_sorted
                x2 = x2_sorted
        ...
        if len(x1.shape) == 2:
            ...
            if dim == 2 :
                ...
                x1_sorted[:, :int(dim/2)], _ = torch.sort(x1[:,:int(dim/2)])
                x2_sorted[:, :int(dim/2)], _ = torch.sort(x2[:,:int(dim/2)])
                # Also sorts time parameters from low to high
                x1_sorted[:, int(dim / 2):], _ = torch.sort(x1[:, int(dim / 2):])
                x2_sorted[:, int(dim / 2):], _ = torch.sort(x2[:, int(dim / 2):])
                x1 = x1_sorted
                x2 = x2_sorted
